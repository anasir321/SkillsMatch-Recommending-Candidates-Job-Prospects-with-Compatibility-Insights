...
// const CareerOfficerJobsArea = ({ job }: { job: IJobType }) => {
...
// export default CareerOfficerJobsArea;
...
// const CareerOfficerJobsArea = ({ job }: { job: IJobType }) => {
//   const [careerOfficersJobs, setCareerOfficersJobs] = useState([]);
...
//         );
//         if (response.status === 200) {
//             // console.log("Career Officer's Jobs: ", response.data)
//             setCareerOfficersJobs(response.data);
//             console.log("Career Officer's Jobs: ", careerOfficersJobs)
//         }
//       } catch (error) {
//         console.error("Error fetching career officer's jobs: ", error);
//       }
//     };

//     fetchCareerOfficersJobs();
//   }, []);

//   return (
//     <section className="career-officers-jobs py-12 bg-white">
//       <div className="container mx-auto">
//         {/* <h2 className="text-3xl font-semibold mb-8">Career Officer's Jobs</h2> */}
//         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//           {careerOfficersJobs.map((job) => (
//             <div key={job.subject} className="bg-gray-100 p-6 rounded-lg">
//               <h3 className="text-xl font-semibold mb-2">{job.subject}</h3>
//               <p className="text-gray-700 mb-4">{job.body}</p>
//               {/* <p className="text-gray-700 mb-2">Location: {job.job_location}</p>
//               <p className="text-gray-700 mb-2">Salary: ${job.salary}/month</p>
//               <p className="text-gray-700 mb-2">Job Type: {job.job_type}</p>
//               <p className="text-gray-700 mb-2">Experience Required: {job.work_experience_required} years</p> */}
...
// Define the CareerOfficerJobsArea component
const CareerOfficerJobsArea = ({ job }: { job: IJobType }) => {
  // State to store career officer's jobs
  const [careerOfficersJobs, setCareerOfficersJobs] = useState([]);

  // Function to format the text with proper HTML elements
  ...
  // Fetch career officer's jobs from the backend
  useEffect(() => {
    const fetchCareerOfficersJobs = async () => {
      try {
        const response = await axios.get(
  ...
  return (
    ...
    //     {careerOfficersJobs.map((job: any, index: number) => (
    //       <div key={index} className="bg-gray-100 p-6 rounded-lg mb-6">
    //         {/* Display the subject of the job */}
    //         <h3 className="text-xl font-semibold mb-2">{job.subject}</h3>
    //         {/* Display the body of the job with proper formatting */}
    //         <div className="text-gray-700 mb-4">{formatText(job.body)}</div>
    //         {/* Add more job details as needed */}
    //       </div>
    //     ))}
    //   </div>
    ...
    //                 <div className="details-post-data me-xxl-5 pe-xxl-4">
    //                 {careerOfficersJobs.map((job: any, index: number) => (
    //                 <div key={index} className="job-title d-flex align-items-center">
    //                     {/* Display the subject of the job */}
    //                     <h3 className="post-title">{job.subject}</h3>
    //                     {/* Display the body of the job with proper formatting */}
    //                     <div className="post-block border-style mt-40 lg-mt-30">{formatText(job.body)}</div>
    //                     {/* Add more job details as needed */}
    //                 </div>
    //                 ))}
    //                 </div>
...
//         <div className="col-xxl-9 col-xl-8">
//             {careerOfficersJobs.map((job: any, index: number) => (
//                 <div key={index} className="details-post-data me-xxl-5 pe-xxl-4">
//                     {/* Display the subject of the job */}
//                     <h3 className="post-title">{job.subject}</h3>
//                     {/* Display the body of the job with proper formatting */}
//                     <div className="post-block border-style mt-40 lg-mt-30">{formatText(job.body)}</div>
...
<section className="job-details pt-100 lg-pt-80 pb-130 lg-pb-80 d-flex justify-content-center align-items-center">
    <div className="container">
        <div className="row">
            <div className="col-xxl-9 col-xl-8">
                {careerOfficersJobs.map((job: any, index: number) => (
                    <div key={index} className="details-post-data me-xxl-5 pe-xxl-4">
                        {/* Display the subject of the job */}
                        <h3 className="post-title mt-4">{job.subject}</h3>
                        {/* Display the body of the job with proper formatting */}
                        <div className="post-block border-style mt-4 lg-mt-3">{formatText(job.body)}</div>
                        {/* Add more job details as needed */}
                    </div>
                ))}
            </div>
...
// Export the CareerOfficerJobsArea component
export default CareerOfficerJobsArea;
